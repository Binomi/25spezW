...
set ylabel 'Leistung $c_m$  [$\si[per-mode=frac]{\joule\per\kelvin\mol}$]'
set key top left
...
p 'heizenAl.dat' u (T($2)):(3*(10.5+0.25*$0)*0.5/(Al_m*(Al_a+Al_l*T($2)))*Al_M):(Al_M/Al_m*sqrt((1.2*0.5/(Al_a+Al_l*T($2)))**2+(3*(10.5+0.25*$0)*0.5/(Al_a+Al_l*T($2)))**2*(Al_a_err**2+(T($2)*Al_l_err)**2+(Al_l*U($2)*V)**2))) w e t'Aluminium',\
  ...
  'StickstoffAlHeizen.dat' u (T($2)):($3*0.5/(Al_m*(N_Al_a+N_Al_l*T($2)))*Al_M):(Al_M/Al_m*sqrt((1.2*0.5/(N_Al_a+N_Al_l*T($2)))**2+($3*0.5/(N_Al_a+N_Al_l*T($2)))**2*(N_Al_a_err**2+(T($2)*N_Al_l_err)**2+(N_Al_l*U($2)*V)**2))) w e pt 1 lt 1 notitle,\
